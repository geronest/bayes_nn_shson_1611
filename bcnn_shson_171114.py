...
class bcnn_conv_g(object):
    
    def __init__(self, inp, shape, mu, rhos, n_samples, rseed, rhoact = softplus, pooling = True, outact = tf.sigmoid, layernum = 0, train_rho = True):
        with tf.name_scope('conv_layer' + str(layernum)):
            ...
            self.e = tf.random_normal([self.n_samples, shape[0], shape[1], shape[2], shape[3]], seed = rseed)           
            ...
            #self.conv = tf.nn.conv2d(inp, self.c_w, strides = [1, 1, 1, 1], padding = 'SAME')
            self.conv = tf.map_fn(lambda (a, b): tf.nn.conv2d(a, b, strides = [1, 1, 1, 1], padding = 'SAME'), (inp, self.c_w), dtype = tf.float32)
            self.pre_o = outact(self.conv)
            
            if pooling:
                self.pool = tf.map_fn(lambda a: tf.nn.max_pool(a, ksize = [1, 2, 2, 1], strides = [1, 2, 2, 1], padding = 'SAME'), self.pre_o, dtype = tf.float32)
                self.out = self.pool
            else:
                self.out = self.pre_o
...
class bcnn_fc_g(object):
    
    def __init__(self, inp, shape, mu, rhos, n_samples, rseed, rhoact = softplus, outact = tf.sigmoid, layernum = 0, train_rho = True):
...
class bcnn_model_g(object):
    
    def __init__(self, shape_img, shape_conv, pool_conv, shape_fc, mu = 0.1, rhos = [0.1, 1.0, 10.0], n_samples = 10, outact = tf.nn.relu, seed = 1234, lr = 1e-8, train_rho = True, only_loglike = False, ewc = False):
        ...
        self.x3 = tf.tile(tf.expand_dims(self.x, 0), [self.n_samples, 1, 1, 1, 1])
        ...
        for i in range(self.n_layers_conv):
            ...
            self.layers.append(bcnn_conv_g(inp, shape_conv[i], mu, rhos, rseed = seed, pooling = pool_conv[i], n_samples = self.n_samples, outact = outact, layernum = i))
        
        for i in range(self.n_layers_fc):
            if i == 0:
                print self.layers[self.n_layers_conv-1].out.get_shape()
                inp = tf.reshape(self.layers[self.n_layers_conv - 1].out, [self.n_samples, -1, shape_fc[0]])
                print inp.get_shape()
            else:
            ...
            self.layers.append(bcnn_fc_g(inp, [shape_fc[i], shape_fc[i+1]], mu, rhos, n_samples = self.n_samples, rseed = seed, outact = actout, layernum = self.n_layers_conv + i))
        ...
        with tf.name_scope('terminal'):
            self.loglike = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits = tf.reshape(self.pred, [-1, shape_fc[-1]]), labels = tf.tile(self.t, [self.n_samples, 1])), name = 'loglike')
