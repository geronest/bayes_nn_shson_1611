...
class exp_manager(object):
    ...
    def train(self, name_train, n_epochs, patience = 3, vdec_range = 20, fs_decay = False, fs_range = 25,\
              update_prior = False, dropout = False, ewc = False, l2_reg = False, supgrad = False, reset_q_params = False, pri_type = 0, kind_model = 'bnn', kind_prob = 'permute', klrw = False): # kind_prob is either 'permute' or 'split'
        ...
        for d in range(n_datas):
            ...
            for ep in range(n_epochs):
                ...
                if supgrad and (ep % (n_epochs/5) == 0): self.model.print_params(file_rec)
...
if __name__ == "__main__":
    ...
    expmng.assign_model(model_bnn)
    tasks.append("SpGr_1_UR_a1_{}".format(init_lr))
    ...
    model_bnn = bnn_model([784, 100, 10], size_data = len(expmng.t_train[0]), size_batch = batch_size, \
                ...
                pri_type = 0, pri_coeff = 1) # squared_std?
    expmng.assign_model(model_bnn)
    tasks.append("SpGr_1_UR_a2_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, patience = patience, vdec_range = vd_range, supgrad = True, update_prior = True, reset_q_params = True, kind_prob = 'permute')
    ...
    expmng.assign_model(model_bnn)
    tasks.append("SpGr_1_UR_a2_p2_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, patience = patience, vdec_range = vd_range, supgrad = True, update_prior = True, reset_q_params = True, kind_prob = 'permute')
    ...
    model_bnn = bnn_model([784, 100, 10], size_data = len(expmng.t_train[0]), size_batch = batch_size, \
                ...
                pri_type = 0, pri_coeff = 1) # squared_std?
    expmng.assign_model(model_bnn)
    tasks.append("SpGr_1_NUR_a1_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, patience = patience, vdec_range = vd_range, supgrad = True, update_prior = True, reset_q_params = False, kind_prob = 'permute')
    ...
    model_bnn = bnn_model([784, 100, 10], size_data = len(expmng.t_train[0]), size_batch = batch_size, \
                ...
                pri_type = 0, pri_coeff = 1) # squared_std?
    expmng.assign_model(model_bnn)
    tasks.append("SpGr_1_NUR_a2_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, patience = patience, vdec_range = vd_range, supgrad = True, update_prior = True, reset_q_params = False, kind_prob = 'permute')
    ...
    expmng.assign_model(model_bnn)
    expmng.train(name_train ="RBE_1_UR_{}_".format(init_lr), n_epochs = num_epochs, patience = patience, vdec_range = vd_range, supgrad = False, update_prior = True, reset_q_params = True)
    ...
    tasks.append("EWC_1_p1_{}".format(init_lr))
    ...
    expmng.assign_model(model_nn)
    tasks.append("EWC_1_p10_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, kind_model = "nn", patience = patience, vdec_range = vd_range, supgrad = False, update_prior = True, ewc = True, l2_reg = False, reset_q_params = False)
    ...
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, kind_model = "nn", patience = patience, vdec_range = vd_range, supgrad = False, update_prior = False, ewc = False, l2_reg = False, reset_q_params = False, kind_prob = 'permute')
    
    init_lr = 1e-5
    num_epochs = 1000 # 400 for bnn, 2000 for nn
    
    # DNN + L2
    expmng.init_session()
    model_nn = nn_shson.nn_model([784, 100, 10], size_data = len(expmng.t_train[0]), size_batch = batch_size, \
                mu = 0.02, outact = tf.nn.relu, seed = 1234, \
                lr = init_lr, ewc = False, l2_reg = True, reg_penalty = 5)
    expmng.assign_model(model_nn)
    tasks.append("L2_1_p5_{}".format(init_lr))
    expmng.train(name_train = tasks[-1], n_epochs = num_epochs, kind_model = "nn", patience = patience, vdec_range = vd_range, supgrad = False, update_prior = False, ewc = False, l2_reg = True, reset_q_params = False, kind_prob = 'permute')
