# Matrix Gaussian based Bayesian Neural Network
    ...
    def __init__(self, inp, shape, sb, mu, rhos, n_samples, rseed, rhoact = softplus, outact = tf.sigmoid, layernum = 0, train_rho = True):
        with tf.name_scope('layer' + str(layernum)):
            ...
            with tf.name_scope('q_pos'):
                ...
                self.u = tf.Variable(tf.constant(rhos[0], shape = [shape[0], 1]), dtype = tf.float32, name = 'u', trainable = train_rho)
                self.v = tf.Variable(tf.constant(rhos[0], shape = [1, shape[1]]), dtype = tf.float32, name = 'v', trainable = train_rho)
                #self.v = tf.Variable(tf.constant(rhos[0], shape = shape[1]), dtype = tf.float32, name = 'v', trainable = train_rho)
                #self.u = tf.Variable(tf.constant(rhos[0], shape = shape[0]), dtype = tf.float32, name = 'u', trainable = train_rho)
                self.n_samples = n_samples
                variable_summaries(self.w)
                variable_summaries(self.u)
                variable_summaries(self.v)

            with tf.name_scope('p_pri'):
                self.p_w = tf.Variable(tf.constant(0.0, shape = shape), trainable = False, dtype = tf.float32, name = 'p_mu')
                self.p_u = tf.Variable(tf.constant(rhos[1], shape = [shape[0], 1]), trainable = False, dtype = tf.float32, name = 'p_v')
                self.p_v = tf.Variable(tf.constant(rhos[1], shape = [1, shape[1]]), trainable = False, dtype = tf.float32, name = 'p_u')
                self.p_bgu = tf.Variable(tf.constant(rhos[2], shape = [shape[0], 1]), trainable = False, dtype = tf.float32)
                self.p_bgv = tf.Variable(tf.constant(rhos[2], shape = [1, shape[1]]), trainable = False, dtype = tf.float32)
                variable_summaries(self.p_w)
                variable_summaries(self.p_u)
                variable_summaries(self.p_v)

            self.uv = tf.matmul(tf.sqrt(rhoact(u)), tf.sqrt(rhoact(v)))
            self.puv = tf.matmul(tf.sqrt(rhoact(p_u)), tf.sqrt(rhoact(p_v)))
            self.pbguv = tf.matmul(tf.sqrt(rhoact(p_bgu)), tf.sqrt(rhoact(p_bgv)))
            
            self.e = tf.random_normal([self.n_samples, shape[0], shape[1]], seed = rseed)           
            self.c_w = self.w + tf.multiply(self.e, self.uv)
            self.q_pos = normpdf(self.c_w, self.w, self.uv)
            self.p_pri = 0.8 * normpdf(self.c_w, self.p_w, self.puv)\
                         + 0.2 * normpdf(self.c_w, self.p_w, self.pbguv)
            ...
            self.params = [self.w, self.u, self.v]
            self.p_params = [self.p_w, self.p_u, self.p_v]

            print ("layer done")

class mbnn_model(object):
    ...
    def __init__(self, shape, size_data, size_batch, mu = 0.1, rhos = [0.1, 1.0, 10.0], n_samples = 10, outact = tf.sigmoid, seed = 1234, lr = 1e-8, kl_reweight = True, train_rho = True, only_loglike = False, ewc = False):
        ...
        for i in range(self.n_layers):
            ...
            self.layers.append(mbnn_layer(inp, [shape[i], shape[i+1]], size_batch, mu, rhos, n_samples = self.n_samples, rseed = seed + i, outact = actout, layernum = i, train_rho = train_rho))
        ...
        if ewc:
            ...
            self.ewc_grad = list()
            for i in range(len(self.train_grad) / 3):
                
                self.ewc_grad.append((self.train_grad[3*i][0] * tf.matmul(softplus(self.p_params[3*i+1]), softplus(self.p_params[3*i+2])), self.train_grad[3*i][1])) # grad_mu * softplus(prior_rho)**2
                
                self.ewc_grad.append(self.train_grad[3*i+1])
                self.ewc_grad.append(self.train_grad[3*i+2])
    ...
    def print_params(self):
        for layer in self.layers:
            ...
            print "u: {}".format(tf.reduce_mean(layer.params[1]).eval())
            print "v: {}".format(tf.reduce_mean(layer.params[2]).eval())
            ...
            print "p_u: {}".format(tf.reduce_mean(layer.p_u).eval())
            print "p_v: {}".format(tf.reduce_mean(layer.p_v).eval())
    ...
    def update_prior(self):
        for layer in self.layers:
            ...
            layer.p_u.assign(layer.u).eval()
            layer.p_v.assign(layer.v).eval()
